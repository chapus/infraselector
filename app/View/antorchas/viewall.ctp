...
//debug($data);
//debug($this->data);
	echo $this->Form->create('Antorcha', array(
...
<div class="admin" align="center">
	<?= $this->Html->link("<button class='rounded' id='add'><span>Agregar Nueva</span></button>", array('controller' => 'antorchas', 'action' => 'add'), array('escape' => false), null); ?>
    <br />
...
?>
        <tr class="<?= $trow; ?>" id="<?= "i".$dat['Antorcha']['id']; ?>">
          <td><?php 
		  if($dat['Antorcha']['image'] != "") {
		  	echo $this->Html->link($this->Html->image('/'.$dat['Antorcha']['smallimage'], array('class' => 'imagen')), '#', array('escape' => false, 'class' => 'tooltip', 'rel' => 'viewimage/'.$dat['Antorcha']['id'], 'title' => $dat['Antorcha']['short']), null);
		  } else {
			echo $this->Html->image('fotos/no_image.png', array('class' => 'imagen'));
		  }
		  ?></td>
          <td><?= $dat['Antorcha']['codigo']; ?></td>
          <td><?= $dat['Antorcha']['name']; ?></td>
          <td><?= $dat['Antorcha']['short']; ?></td>
          <td><?php 
		  
			$body = strip_tags($dat['Antorcha']['description']); 
		  ...
		  ?></td>
          <td><?php 
		  if($dat['Antorcha']['infra_link'] != "") {
		  echo $this->Html->link(
		  $this->Html->image('icons/attach.png', array('class' => 'icons')), 
		  $dat['Antorcha']['infra_link'], array('escape' => false, 'target' => '_blank'), null);
		  } else {
			  echo "-";
		  }
		  ?>
          ...
          <td><?php 
		  if($dat['Antorcha']['pmig'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso MIG'));
		  } else { echo "-"; }
		  ?>
          </td>
          <td><?php 
		  if($dat['Antorcha']['ptig'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso TIG'));
		  } else { echo "-"; }
		  ?>
          </td>
          <td><?php 
		  if($dat['Antorcha']['psmaw'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso Electrodo Revestido'));
		  } else { echo "-"; }
		  ?>
          </td>
          
          <td><?= date("F j, Y", strtotime($dat['Antorcha']['created'])); ?></td>
          
          <td><?php 
		  echo $this->Html->link('Ver', array('controller' => '', 'action' => 'lasantorchas', $dat['Antorcha']['id']))."<br />"; 
		  echo $this->Html->link('Editar', array('controller' => 'antorchas', 'action' => 'edit', $dat['Antorcha']['id']))."<br />"; 
		  ?></td>
        </tr>
<?php
}
...
'format' => __('Página %page% de %pages%, mostrando %current% antorchas de lámina de %count% en total, empezando del calibre de lámina %start%, al %end%', true)
));
?>	</p>
	<div class="paging">
