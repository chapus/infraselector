...
<?php echo $this->Form->create('Antorcha', array('type' => 'file'));
		...
		echo $this->Form->input('name', array('label' => 'Nombre Completo')).$this->Html->tag('div', 'Nombre completo de la Antorcha','inputmsg');
		
		echo $this->Form->input('short', array('label' => 'Nombre Corto')).$this->Html->tag('div', 'Nombre popular o corto de la Antorcha. Nota: este nombre sale debajo del nombre completo en letras pequeñas.','inputmsg');
		...
		echo $this->Form->input('infra_link', array('label' => 'Link Interno')).$this->Html->tag('div', 'El Link interno es opcional, si se tiene información adicional del producto dentro del portal de INFRA se puede poner el link aquí. Este aparecería en la Antorcha como link para mayor información.','inputmsg');
