...
<h3><?= $this->Html->image('infrasmall.png'); ?> Nuevo Artículo de Protección</h3>
...
<?php echo $this->Form->create('Proteccione', array('type' => 'file'));
		...
		echo $this->Form->input('name', array('label' => 'Nombre Completo')).$this->Html->tag('div', 'Nombre completo del Artículo de Protección','inputmsg');
		
		echo $this->Form->input('short', array('label' => 'Nombre Corto')).$this->Html->tag('div', 'Nombre popular o corto del Artículo de Protección. Nota: este nombre sale debajo del nombre completo en letras pequeñas.','inputmsg');
		...
		echo $this->Form->input('infra_link', array('label' => 'Link Interno')).$this->Html->tag('div', 'El Link interno es opcional, si se tiene información adicional del producto dentro del portal de INFRA se puede poner el link aquí. Este aparecería en el Artículo de Protección como link para mayor información.','inputmsg');
...
<div class="submit">
    <button class="rounded" id="submit">
      <span>Agregar Nuevo Artículo de Protección</span>
    </button> 
</div>    
</form> 
</div>
