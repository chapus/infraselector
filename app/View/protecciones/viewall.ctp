...
//debug($data);
//debug($this->data);
	echo $this->Form->create('Proteccione', array(
...
<div class="admin" align="center">
	<?= $this->Html->link("<button class='rounded' id='add'><span>Agregar Nuevo</span></button>", array('controller' => 'protecciones', 'action' => 'add'), array('escape' => false), null); ?>
    <br />
...
<table border="0" class="adm_table">
  <thead class="adm_header">
    <tr>
      ...
      <th><?php echo $this->Paginator->sort('Sección', 'Seccion.name');?></th>
...
?>
        <tr class="<?= $trow; ?>" id="<?= "i".$dat['Proteccione']['id']; ?>">
          <td><?php 
		  if($dat['Proteccione']['image'] != "") {
		  	echo $this->Html->link($this->Html->image('/'.$dat['Proteccione']['smallimage'], array('class' => 'imagen')), '#', array('escape' => false, 'class' => 'tooltip', 'rel' => 'viewimage/'.$dat['Proteccione']['id'], 'title' => $dat['Proteccione']['short']), null);
		  } else {
			echo $this->Html->image('fotos/no_image.png', array('class' => 'imagen'));
		  }
		  ?></td>
          <td><?= $dat['Proteccione']['codigo']; ?></td>
          <td><?= '<b>'.$dat['Seccion']['name'].'</b>'; ?></td>
          <td><?= $dat['Proteccione']['name']; ?></td>
          <td><?= $dat['Proteccione']['short']; ?></td>
          <td><?php 
		  
			$body = strip_tags($dat['Proteccione']['description']); 
		  ...
		  ?></td>
          <td><?php 
		  if($dat['Proteccione']['infra_link'] != "") {
		  echo $this->Html->link(
		  $this->Html->image('icons/attach.png', array('class' => 'icons')), 
		  $dat['Proteccione']['infra_link'], array('escape' => false, 'target' => '_blank'), null);
		  } else {
			  echo "-";
		  }
		  ?>
          ...
          <td><?php 
		  if($dat['Proteccione']['pmig'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso MIG'));
		  } else { echo "-"; }
		  ?>
          </td>
          <td><?php 
		  if($dat['Proteccione']['ptig'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso TIG'));
		  } else { echo "-"; }
		  ?>
          </td>
          <td><?php 
		  if($dat['Proteccione']['psmaw'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso Electrodo Revestido'));
		  } else { echo "-"; }
		  ?>
          </td>
          <td><?php 
		  if($dat['Proteccione']['ppac'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso Corte por Plasma'));
		  } else { echo "-"; }
		  ?>
          </td>
          
          <td><?= date("F j, Y", strtotime($dat['Proteccione']['created'])); ?></td>
          
          <td><?php 
		  echo $this->Html->link('Ver', array('controller' => '', 'action' => 'articulosdeproteccion', $dat['Proteccione']['id']))."<br />"; 
		  echo $this->Html->link('Editar', array('controller' => 'protecciones', 'action' => 'edit', $dat['Proteccione']['id']))."<br />"; 
		  ?></td>
        </tr>
<?php
}
...
'format' => __('Página %page% de %pages%, mostrando %current% protecciones de lámina de %count% en total, empezando del calibre de lámina %start%, al %end%', true)
));
?>	</p>
	<div class="paging">
