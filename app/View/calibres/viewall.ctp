...
//debug($data);
//debug($this->data);
	echo $this->Form->create('Calibre', array(
...
<div class="admin" align="center">
	<?= $this->Html->link("<button class='rounded' id='add'><span>Agregar Nuevo</span></button>", array('controller' => 'calibres', 'action' => 'add'), array('escape' => false), null); ?>
    <br />
...
'format' => __('Página %page% de %pages%, mostrando %current% calibres de lámina de %count% en total, empezando del calibre de lámina %start%, al %end%', true)
));
?>	</p>
	<div class="paging">
...
?>
        <tr class="<?= $trow; ?>" id="<?= "i".$dat['Calibre']['id']; ?>">
          <td><?php 
		  if($dat['Calibre']['image'] != "") {
		  	echo $this->Html->image('/'.$dat['Calibre']['image'], array('class' => 'imagen'));
		  } else {
			echo $this->Html->image('fotos/no_image.png', array('class' => 'imagen'));
		  }
		  ?></td>
          <td><?= $dat['Calibre']['name']; ?></td>
          <td><?= $dat['Calibre']['short']; ?></td>
          <td><?php 
		  
			$body = strip_tags($dat['Calibre']['description']); 
		  ...
		  ?></td>
          <td><?php 
		  if($dat['Calibre']['infra_link'] != "") {
		  echo $this->Html->link(
		  $this->Html->image('icons/attach.png', array('class' => 'icons')), 
		  $dat['Calibre']['infra_link'], array('escape' => false, 'target' => '_blank'), null);
		  } else {
			  echo "-";
		  }
		  ?>
          ...
          <td><?php 
		  if($dat['Calibre']['pmig'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso MIG'));
		  } else { echo "-"; }
		  ?>
          </td>
          <td><?php 
		  if($dat['Calibre']['ptig'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso TIG'));
		  } else { echo "-"; }
		  ?>
          </td>
          <td><?php 
		  if($dat['Calibre']['psmaw'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso Electrodo Revestido'));
		  } else { echo "-"; }
		  ?>
          </td>
          <td><?php 
		  if($dat['Calibre']['ppac'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso Corte por Plasma'));
		  } else { echo "-"; }
		  ?>
          </td>
          
          <td><?= date("F j, Y", strtotime($dat['Calibre']['created'])); ?></td>
          
          <td><?php 
		  echo $this->Html->link('Ver', array('controller' => '', 'action' => 'calibresdelamina', $dat['Calibre']['id']))."<br />"; 
		  echo $this->Html->link('Editar', array('controller' => 'calibres', 'action' => 'edit', $dat['Calibre']['id'].'/'.str_replace(array('/', ':'), array('_', '-'), $this->params['url']['url']) ))."<br />"; 
		  ?></td>
        </tr>
<?php
}
