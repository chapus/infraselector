...
<h3><?= $this->Html->image('infrasmall.png'); ?> condicion de calidad</h3>
<p>&nbsp;</p>
<div class="search">
...
//debug($data);
//debug($this->data);
	echo $this->Form->create('Ccalidade', array(
...
<div class="admin" align="center">
	<?= $this->Html->link("<button class='rounded' id='add'><span>Agregar Nuevo</span></button>", array('controller' => 'ccalidades', 'action' => 'add'), array('escape' => false), null); ?>
    <br />
...
?>
        <tr class="<?= $trow; ?>" id="<?= "i".$dat['Ccalidade']['id']; ?>">
          <td><?php 
		  if($dat['Ccalidade']['image'] != "") {
		  	echo $this->Html->link($this->Html->image('/'.$dat['Ccalidade']['smallimage'], array('class' => 'imagen')), '#', array('escape' => false, 'class' => 'tooltip', 'rel' => 'viewimage/'.$dat['Ccalidade']['id'], 'title' => $dat['Ccalidade']['short']), null);
		  } else {
			echo $this->Html->image('fotos/no_image.png', array('class' => 'imagen'));
		  }
		  ?></td>
          <td><?= $dat['Ccalidade']['codigo']; ?></td>
          <td><?= $dat['Ccalidade']['name']; ?></td>
          <td><?= $dat['Ccalidade']['short']; ?></td>
          <td><?php 
		  
			$body = strip_tags($dat['Ccalidade']['description']); 
		  ...
		  ?></td>
          <td><?php 
		  if($dat['Ccalidade']['infra_link'] != "") {
		  echo $this->Html->link(
		  $this->Html->image('icons/attach.png', array('class' => 'icons')), 
		  $dat['Ccalidade']['infra_link'], array('escape' => false, 'target' => '_blank'), null);
		  } else {
			  echo "-";
		  }
		  ?>
          ...
          <td><?php 
		  if($dat['Ccalidade']['pmig'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso MIG'));
		  } else { echo "-"; }
		  ?>
          </td>
          <td><?php 
		  if($dat['Ccalidade']['ptig'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso TIG'));
		  } else { echo "-"; }
		  ?>
          </td>
          <td><?php 
		  if($dat['Ccalidade']['psmaw'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso Electrodo Revestido'));
		  } else { echo "-"; }
		  ?>
          </td>
          
          <td><?= date("F j, Y", strtotime($dat['Ccalidade']['created'])); ?></td>
          
          <td><?php 
		  echo $this->Html->link('Ver', array('controller' => '', 'action' => 'materialesdeaporte', $dat['Ccalidade']['id']))."<br />"; 
		  echo $this->Html->link('Editar', array('controller' => 'ccalidades', 'action' => 'edit', $dat['Ccalidade']['id'].'/'.str_replace(array('/', ':'), array('_', '-'), $this->request->url) ))."<br />"; 
		  ?></td>
        </tr>
<?php
}
...
'format' => __('Página %page% de %pages%, mostrando %current% ccalidades de lámina de %count% en total, empezando del calibre de lámina %start%, al %end%')
));
?>	</p>
	<div class="paging">
