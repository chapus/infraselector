...
//debug($data);
//debug($this->data);
	echo $this->Form->create('Regulador', array(
...
<div class="admin" align="center">
	<?= $this->Html->link("<button class='rounded' id='add'><span>Agregar Nuevo</span></button>", array('controller' => 'reguladors', 'action' => 'add'), array('escape' => false), null); ?>
    <br />
...
?>
        <tr class="<?= $trow; ?>" id="<?= "i".$dat['Regulador']['id']; ?>">
          <td><?php 
		  if($dat['Regulador']['image'] != "") {
		  	echo $this->Html->link($this->Html->image('/'.$dat['Regulador']['smallimage'], array('class' => 'imagen')), '#', array('escape' => false, 'class' => 'tooltip', 'rel' => 'viewimage/'.$dat['Regulador']['id'], 'title' => $dat['Regulador']['short']), null);
		  } else {
			echo $this->Html->image('fotos/no_image.png', array('class' => 'imagen'));
		  }
		  ?></td>
          <td><?= $dat['Regulador']['codigo']; ?></td>
          <td><?= $dat['Regulador']['name']; ?></td>
          <td><?= $dat['Regulador']['short']; ?></td>
          <td><?php 
		  
			$body = strip_tags($dat['Regulador']['description']); 
		  ...
		  ?></td>
          <td><?php 
		  if($dat['Regulador']['infra_link'] != "") {
		  echo $this->Html->link(
		  $this->Html->image('icons/attach.png', array('class' => 'icons')), 
		  $dat['Regulador']['infra_link'], array('escape' => false, 'target' => '_blank'), null);
		  } else {
			  echo "-";
		  }
		  ?>
          ...
          <td><?php 
		  if($dat['Regulador']['pmig'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso MIG'));
		  } else { echo "-"; }
		  ?>
          </td>
          <td><?php 
		  if($dat['Regulador']['ptig'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso TIG'));
		  } else { echo "-"; }
		  ?>
          </td>
          <td><?php 
		  if($dat['Regulador']['psmaw'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso Electrodo Revestido'));
		  } else { echo "-"; }
		  ?>
          </td>
          <td><?php 
		  if($dat['Regulador']['ppac'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso Corte por Plasma'));
		  } else { echo "-"; }
		  ?>
          </td>
          
          <td><?= date("F j, Y", strtotime($dat['Regulador']['created'])); ?></td>
          
          <td><?php 
		  echo $this->Html->link('Ver', array('controller' => '', 'action' => 'reguladoresdepresion', $dat['Regulador']['id']))."<br />"; 
		  echo $this->Html->link('Editar', array('controller' => 'reguladors', 'action' => 'edit', $dat['Regulador']['id']))."<br />"; 
		  ?></td>
        </tr>
<?php
}
...
'format' => __('Página %page% de %pages%, mostrando %current% reguladors de lámina de %count% en total, empezando del calibre de lámina %start%, al %end%', true)
));
?>	</p>
	<div class="paging">
