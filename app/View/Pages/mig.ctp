...
<div class="title"><b>Selección 10</b></div>
...
$.fn.fClear = function(start, end) {
    for(i = start; i <= end; i++) {
        ...
        if(i <= 8) {
            ...
            if(j == 4) { $("#maquinas").empty(); $('#stp'+i+' .center .info').html('Elija en las siguientes opciones una máquina para soldar de acuerdo al voltaje de alimentación, especificaciones técnicas y tipo de trabajo que desempeñará. El sistema puede darle una sola opción por default de acuerdo a sus necesidades.'); }
            ...
            if(j == 6) { $("#antorchas").removeOption(/./); $('#stp'+i+' .center .info').html('Elija el tipo de antorcha que mejor se adapte a la maquina para soldar seleccionada y a sus procesos de soldadura. El sistema puede darle una sola opción por default de acuerdo a sus necesidades.'); }
            if(j == 7) { $("#aportes").removeOption(/./); $('#stp'+i+' .center .info').html('Elija el material de aporte que requiere para sus procesos de soldadura de acuerdo a los diámetros recomendados. El sistema puede darle una sola opción por default de acuerdo a sus necesidades.'); }
            if(j == 8) { $("#reguladores").removeOption(/./); $('#stp'+i+' .center .info').html('Elija el regulador de presión que requiere el cilindro del gas de protección seleccionado.El sistema puede darle una sola opción por default de acuerdo a sus necesidades.'); }
            if(j == 9) { $("#slider").hide(); }
        }
    }
...
$.fn.fNext = function() {
    ...
    if($('#microalambres option:selected').val() == "") {
        $('#microalambres').focus();
        return false;
    } 
            ...
            var ax_maq = $.ajax({
                    ...
                    success: function (data) {
                        $('#stp4 .left .loader').fadeOut(200, function () {
                            ...
                            $("#microalambres").removeOption(/./);
                            $.fn.fClear(5, 9);
                            for(i = 0; i < data.length; i++) {
                                $.each(data[i], function() {
                                    if(data.length != 1) {
                                        $("#microalambres").addOption(this.id, this.name+' '+this.short, false);
                                    } else {
                                        $("#microalambres").addOption(this.id, this.name+' '+this.short);
                                        $("#microalambres").fMicroalambres();
                                    }
                                });
                                $("#microalambres").focus();
                            }
                    ...
                    error: function(jqXHR, textStatus, errorThrown) {
                        ...
                        if(flag < 5) {
                            ...
                            $('#maquinas option[value=""]').attr('selected', '');
                            $('#stp4 .left .loader').fadeOut(200);
                            $('#stp4 .right .loader').fadeOut(200);
                            $('#stp4 .center .info').html('<font style="color:red;">La aplicación se demoró en mostrar los resultados. Por favor intente de nuevo su selección. Si el problema persiste, contáctenos.</font>');
                        }
                    }
            }); 
...
$.fn.fMicroalambres = function() {
    if($('#microalambres option:selected').val() != "") {
       $('#stp5 .left .loader').show();
            var ax_micro = $.ajax({
                    url: path+'migs/selectorStep6',
                    data: {'id' : $('#microalambres option:selected').val(),
                    ...
                    success: function (data) {
                        $('#stp5 .left .loader').fadeOut(200, function () {
                            $.fn.fClear(6, 9);
                            //$('#stp5 .right .title').html($('#microalambres option:selected').text());
                    ...
                    error: function(jqXHR, textStatus, errorThrown) {
                        ...
                        if(flag < 5) {
                            ...
                            $('#microalambres option[value=""]').attr('selected', '');
            ...
            var ax_micro2 = $.ajax({
                    ...
                    success: function (data) {
                        $('#stp5 .right .loader').fadeOut(200, function () {
                            $.each(data, function() {
                                $('#stp5 .right .title').html(this.name);
    ...
    } else { $.fn.fClearInner( $('#stp5') ); $.fn.fClear(6, 9); $('#stp5 .center .info').html('Elija en las siguientes opciones el alimentador de microalambre que mejor se adapte a sus necesidades, de acuerdo a la máquina para soldar seleccionada anteriormente. El sistema NO puede ofrecerle alguna alternativa si su máquina para soldar ya tiene el alimentador integrado.'); }
}
...
$.fn.fAntorchas = function() {
    if($('#antorchas option:selected').val() != "") {
       $('#stp6 .left .loader').show();
            var ax_ant = $.ajax({
                    url: path+'migs/selectorStep7',
                    data: {'id' : $('#antorchas option:selected').val(),
                    'microid' : $('#microalambres option:selected').val(),
                    'maqid' : $('#maquinas option:selected').val(),
                    'gasid' : $('#gases option:selected').val(),
                    'calibreid' : $('#calibres option:selected').val(),
                    'matid' : $('#materials option:selected').val()},
                    ...
                    success: function (data) {
                        $('#stp6 .left .loader').fadeOut(200, function () {
                            $.fn.fClear(7, 9);
                            $('#stp6 .right .title').html($('#antorchas option:selected').text());
                    ...
                    error: function(jqXHR, textStatus, errorThrown) {
                        ...
                        if(flag < 5) {
                            ...
                            $('#stp6 .left .loader').fadeOut(200);
                            $('#stp6 .right .loader').fadeOut(200);
                            $('#stp6 .center .info').html('<font style="color:red;">La aplicación se demoró en mostrar los resultados. Por favor intente de nuevo su selección. Si el problema persiste, contáctenos.</font>');
                        }
                    }
            });
    ...
    } else { $.fn.fClearInner( $('#stp6') ); $.fn.fClear(7, 9); $('#stp6 .center .info').html('Elija el tipo de antorcha que mejor se adapte a la maquina para soldar seleccionada y a sus procesos de soldadura. El sistema puede darle una sola opción por default de acuerdo a sus necesidades.'); }
}
...
$.fn.fAportes = function() {
            ...
            var ax_apo = $.ajax({
                    ...
                    success: function (data) {
                        $('#stp7 .left .loader').fadeOut(200, function () {
                            $.fn.fClear(8, 9);
                            $('#stp7 .right .title').html($('#aportes option:selected').text());
                    ...
                    error: function(jqXHR, textStatus, errorThrown) {
                        ...
                        if(flag < 5) {
                            ...
                            $('#aportes option[value=""]').attr('selected', '');
                            $('#stp7 .left .loader').fadeOut(200);
                            $('#stp7 .right .loader').fadeOut(200);
                            $('#stp7 .center .info').html('<font style="color:red;">La aplicación se demoró en mostrar los resultados. Por favor intente de nuevo su selección. Si el problema persiste, contáctenos.</font>');
                        }
                    }
            }); 
    ...
    } else { $.fn.fClearInner( $('#stp7') ); $.fn.fClear(8, 9); $('#stp7 .center .info').html('Elija el material de aporte que requiere para sus procesos de soldadura de acuerdo a los diámetros recomendados. El sistema puede darle una sola opción por default de acuerdo a sus necesidades.'); }
}
...
$.fn.fReguladores = function() {
    if($('#reguladores option:selected').val() != "") {
       $('#stp8 .left .loader').show();
       
            $.ajax({
                url: path+'migs/alternativos',
                ...
                success: function (data) {
                    $('#tab_alt1').empty();
                    $('#stp8 .left .loader').fadeOut(200, function () {
                        //$.fn.fClear(8, 9);
                        //$('#stp8 .right .title').html($('#reguladores option:selected').text());
                        for(i = 0; i < data.length; i++) {
                            $.each(data[i], function() {
                                if(this.name != "N/A") {
            ...
            var ax_reg = $.ajax({
                        ...
                        $('#stp8 .left .loader').fadeOut(200, function () {
                            ...
                            for(i = 0; i < data.length; i++) {
                                $.each(data[i], function() {
                                    ...
                                    $('#tabs').find('ul').append('<li><a href="'+path+'seccions/getMig/'+this.id+'">'+this.name+'</a></li>');
                    ...
                    error: function(jqXHR, textStatus, errorThrown) {
                        ...
                        if(flag < 5) {
                            ...
                            $('#reguladores option[value=""]').attr('selected', '');
                            $('#stp8 .left .loader').fadeOut(200);
                            $('#stp8 .right .loader').fadeOut(200);
                            $('#stp8 .center .info').html('<font style="color:red;">La aplicación se demoró en mostrar los resultados. Por favor intente de nuevo su selección. Si el problema persiste, contáctenos.</font>');
                        }
                    }
            }); 
            $('#stp8 .right .image').html('');
            $('#stp8 .right .loader').show();
            var ax_reg2 = $.ajax({
                url: path+'migs/interStep9',
                data: {'id' : $('#reguladores option:selected').val()},
                type: 'POST',
                dataType: 'json',
                success: function (data) {
                    $('#stp8 .right .loader').fadeOut(200, function () {
                        $.each(data, function() {
                            $('#stp8 .center .info').html(this.description);
                            $('#stp8 .right .image').html('<img src="'+this.smallimage+'" />');
                            });
            ...
            $('div#submit').addClass('rdy');    
            
    } else { $.fn.fClearInner( $('#stp8') ); $('#stp8 .center .info').html('Elija el regulador de presión que requiere el cilindro del gas de protección seleccionado.El sistema puede darle una sola opción por default de acuerdo a sus necesidades.'); }
}
...
$('div#submit').click(function () {
    ...
    if(valid == true) {
        ...
        var cadena = "/mat/"+$('#materials option:selected').val()+"/cal/"+$('#calibres option:selected').val()+"/gas/"+$('#gases option:selected').val()+"/maq/"+$('#maquinas option:selected').val()+"/micro/"+$('#microalambres option:selected').val()+"/ant/"+$('#antorchas option:selected').val()+"/apo/"+$('#aportes option:selected').val()+"/reg/"+$('#reguladores option:selected').val()+alterns+protect;
        
        document.location.href= path+'mig/step2'+cadena;
    } else {
