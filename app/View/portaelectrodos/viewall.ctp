...
//debug($data);
//debug($this->data);
	echo $this->Form->create('Portaelectrodo', array(
...
<div class="admin" align="center">
	<?= $this->Html->link("<button class='rounded' id='add'><span>Agregar Nuevo</span></button>", array('controller' => 'portaelectrodos', 'action' => 'add'), array('escape' => false), null); ?>
    <br />
...
?>
        <tr class="<?= $trow; ?>" id="<?= "i".$dat['Portaelectrodo']['id']; ?>">
          <td><?php 
		  if($dat['Portaelectrodo']['smallimage'] != "") {
		  	echo $this->Html->link($this->Html->image('/'.$dat['Portaelectrodo']['smallimage'], array('class' => 'imagen')), '#', array('escape' => false, 'class' => 'tooltip', 'rel' => 'viewimage/'.$dat['Portaelectrodo']['id'], 'title' => $dat['Portaelectrodo']['short']), null);
		  } else {
			echo $this->Html->image('fotos/no_image.png', array('class' => 'imagen'));
		  }
		  ?></td>
          <td><?= $dat['Portaelectrodo']['codigo']; ?></td>
          <td><?= $dat['Portaelectrodo']['name']; ?></td>
          <td><?= $dat['Portaelectrodo']['short']; ?></td>
          <td><?php 
		  
			$body = strip_tags($dat['Portaelectrodo']['description']); 
		  ...
		  ?></td>
          <td><?php 
		  if($dat['Portaelectrodo']['infra_link'] != "") {
		  echo $this->Html->link(
		  $this->Html->image('icons/attach.png', array('class' => 'icons')), 
		  $dat['Portaelectrodo']['infra_link'], array('escape' => false, 'target' => '_blank'), null);
		  } else {
			  echo "-";
		  }
		  ?>
          ...
          <td><?php 
		  if($dat['Portaelectrodo']['ppac'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso Corte por Plasma'));
		  } else { echo "-"; }
		  ?>
          </td>
          
          <td><?= date("F j, Y", strtotime($dat['Portaelectrodo']['created'])); ?></td>
          
          <td><?php 
		  echo $this->Html->link('Ver', array('controller' => '', 'action' => 'portaelectrodo', $dat['Portaelectrodo']['id']))."<br />"; 
		  echo $this->Html->link('Editar', array('controller' => 'portaelectrodos', 'action' => 'edit', $dat['Portaelectrodo']['id']))."<br />"; 
		  ?></td>
        </tr>
<?php
}
...
'format' => __('Página %page% de %pages%, mostrando %current% gases de protección de %count% en total, empezando del gas de protección %start%, al %end%', true)
));
?>	</p>
	<div class="paging">
