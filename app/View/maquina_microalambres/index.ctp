<div class="maquinaMicroalambres index">
	<h2><?php __('Maquina Microalambres');?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id');?></th>
			<th><?php echo $this->Paginator->sort('maquina_id');?></th>
			<th><?php echo $this->Paginator->sort('microalambre_id');?></th>
			<th class="actions"><?php __('Actions');?></th>
	</tr>
	<?php
	$i = 0;
	foreach ($maquinaMicroalambres as $maquinaMicroalambre):
		$class = null;
		if ($i++ % 2 == 0) {
			$class = ' class="altrow"';
		}
	?>
	<tr<?php echo $class;?>>
		<td><?php echo $maquinaMicroalambre['MaquinaMicroalambre']['id']; ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($maquinaMicroalambre['Maquina']['name'], array('controller' => 'maquinas', 'action' => 'view', $maquinaMicroalambre['Maquina']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($maquinaMicroalambre['Microalambre']['name'], array('controller' => 'microalambres', 'action' => 'view', $maquinaMicroalambre['Microalambre']['id'])); ?>
		</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View', true), array('action' => 'view', $maquinaMicroalambre['MaquinaMicroalambre']['id'])); ?>
			<?php echo $this->Html->link(__('Edit', true), array('action' => 'edit', $maquinaMicroalambre['MaquinaMicroalambre']['id'])); ?>
			<?php echo $this->Html->link(__('Delete', true), array('action' => 'delete', $maquinaMicroalambre['MaquinaMicroalambre']['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $maquinaMicroalambre['MaquinaMicroalambre']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page %page% of %pages%, showing %current% records out of %count% total, starting on record %start%, ending on %end%', true)
	));
	?>	</p>

	<div class="paging">
		<?php echo $this->Paginator->prev('<< ' . __('previous', true), array(), null, array('class'=>'disabled'));?>
	 | 	<?php echo $this->Paginator->numbers();?>
 |
		<?php echo $this->Paginator->next(__('next', true) . ' >>', array(), null, array('class' => 'disabled'));?>
	</div>
</div>
<div class="actions">
	<h3><?php __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Maquina Microalambre', true), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Maquinas', true), array('controller' => 'maquinas', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Maquina', true), array('controller' => 'maquinas', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Microalambres', true), array('controller' => 'microalambres', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Microalambre', true), array('controller' => 'microalambres', 'action' => 'add')); ?> </li>
	</ul>
</div>