...
<h3><?= $this->Html->image('infrasmall.png'); ?> secciones para artículos de protección</h3>
<p>&nbsp;</p>
<div class="search">
...
//debug($data);
//debug($this->data);
	echo $this->Form->create('Seccion', array(
		'url' => array_merge(array('action' => 'viewall'), $this->params['pass'])
		));
	echo $this->Form->input('name', array('label' => 'Por Nombre de Sección'));
...
<div class="admin" align="center">
	<?= $this->Html->link("<button class='rounded' id='add'><span>Agregar Nueva Sección</span></button>", array('controller' => 'seccions', 'action' => 'add'), array('escape' => false), null); ?>
    <br />
...
?>
        <tr class="<?= $trow; ?>" id="<?= "i".$dat['Seccion']['id']; ?>">
          <td width="500px"><?= '<b>'.$dat['Seccion']['name'].'</b>'; ?></td>
          
          <td><?= date("F j, Y", strtotime($dat['Seccion']['created'])); ?></td>
          <td><?php 
		  echo $this->Html->link('Editar', array('controller' => 'seccions', 'action' => 'edit', $dat['Seccion']['id'].'/'.str_replace(array('/', ':'), array('_', '-'), $this->params['url']['url']) ))."<br />"; 
		  ?></td>
        </tr>
<?php
}
...
'format' => __('Página %page% de %pages%, mostrando %current% seccions de lámina de %count% en total, empezando del calibre de lámina %start%, al %end%', true)
));
?>	</p>
	<div class="paging">
