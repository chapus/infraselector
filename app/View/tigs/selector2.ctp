...
$(document).ready(function() { 
	...
	$i = 1;
	foreach($alternativos as $alternavito) {
		echo "$('a#valter".$i."').colorbox({inline: true, href:'#dalter".$i."', maxWidth: '500px'});";
		$i++;
	}
	?>
...
<tr>
	...
	<?= $this->Html->link($this->Html->image('icons/back.png').'Vuelva a empezar', '/proceso-tig', array('class' => 'large fancybtn', 'escape' => false) ); ?>
    </div>
    ...
    <div style="float:left; width:40%; padding:5px;">
    <?= $this->Html->link($this->Html->image('icons/email.png').'Enviar por E-mail', 'mailto:?Subject=Selección TIG&body=http://www.infra-selector.com.mx'.$_SERVER['REQUEST_URI'], array('class' => 'large fancybtn', 'escape' => false, 'target' => '_blank') ); ?>
    </div>
    <div style="float:right; width:50%; padding:5px;">
    <?php
	$printpdf = explode('step2', $_SERVER["REQUEST_URI"]);
	echo $this->Html->link($this->Html->image('icons/pdf.png').'Descargar información', '/pdf/tig'.$printpdf[1], array('class' => 'large fancybtn', 'escape' => false, 'target' => '_blank') );
	?>
    </div>
    </td>
</tr>
...
<?php echo $this->Form->input('selector', array('type' => 'hidden', 'value' => 2) ); ?>
</form>
...
<tbody>
    ...
    <?php
	if($aporte['Aporte']['smallimage'] != null ||$aporte['Aporte']['smallimage'] != '') {
	?>
    <tr class="bold" id="aporte"><th><?= $aporte['Aporte']['name']." <span class='pcodimg'>Código  ".$aporte['Aporte']['codigo']." </span>"; ?></th></tr>
    <tr>
        <td><?php 
		echo $aporte['Aporte']['smallimage'] == null ? '' : $this->Html->image('/'.$aporte['Aporte']['smallimage']); ?></td>
    </tr>
    <?php
	}
	?>
...
<div class="clear"></div>
    ...
    <div id="dapor"><?php 
			if($aporte['Aporte']['smallimage'] != null) {
				echo $this->Html->image('/'.$aporte['Aporte']['smallimage'], array('class' => 'imgcentered') ).$aporte['Aporte']['description']; 
			} else {
				echo $aporte['Aporte']['description']; 
			}
		?></div>
...
<?php
	$i = 1;
	foreach($alternativos as $alternativo) {
		echo "<div id='dalter$i'>".$this->Html->image('/'.$alternativo['Alternativo']['smallimage'], array('class' => 'imgcentered') ).$alternativo['Alternativo']['description']."</div>";
		$i++;
	}
?>
    </div>
