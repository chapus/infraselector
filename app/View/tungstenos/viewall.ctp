...
//debug($data);
//debug($this->data);
	echo $this->Form->create('Tungsteno', array(
...
<div class="admin" align="center">
	<?= $this->Html->link("<button class='rounded' id='add'><span>Agregar Nuevo</span></button>", array('controller' => 'tungstenos', 'action' => 'add'), array('escape' => false), null); ?>
    <br />
...
?>
        <tr class="<?= $trow; ?>" id="<?= "i".$dat['Tungsteno']['id']; ?>">
          <td><?php 
		  if($dat['Tungsteno']['image'] != "") {
		  	echo $this->Html->link($this->Html->image('/'.$dat['Tungsteno']['smallimage'], array('class' => 'imagen')), '#', array('escape' => false, 'class' => 'tooltip', 'rel' => 'viewimage/'.$dat['Tungsteno']['id'], 'title' => $dat['Tungsteno']['short']), null);
		  } else {
			echo $this->Html->image('fotos/no_image.png', array('class' => 'imagen'));
		  }
		  ?></td>
          <td><?= $dat['Tungsteno']['codigo']; ?></td>
          <td><?= $dat['Tungsteno']['name']; ?></td>
          <td><?= $dat['Tungsteno']['short']; ?></td>
          <td><?php 
		  
			$body = strip_tags($dat['Tungsteno']['description']); 
		  ...
		  ?></td>
          <td><?php 
		  if($dat['Tungsteno']['infra_link'] != "") {
		  echo $this->Html->link(
		  $this->Html->image('icons/attach.png', array('class' => 'icons')), 
		  $dat['Tungsteno']['infra_link'], array('escape' => false, 'target' => '_blank'), null);
		  } else {
			  echo "-";
		  }
		  ?>
          ...
          <td><?php 
		  if($dat['Tungsteno']['pmig'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso MIG'));
		  } else { echo "-"; }
		  ?>
          </td>
          <td><?php 
		  if($dat['Tungsteno']['ptig'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso TIG'));
		  } else { echo "-"; }
		  ?>
          </td>
          <td><?php 
		  if($dat['Tungsteno']['psmaw'] != 0) {
		  echo $this->Html->image('icons/tick.png', array('class' => 'icons', 'title' => 'Proceso Electrodo Revestido'));
		  } else { echo "-"; }
		  ?>
          </td>
          
          <td><?= date("F j, Y", strtotime($dat['Tungsteno']['created'])); ?></td>
          
          <td><?php 
		  echo $this->Html->link('Ver', array('controller' => '', 'action' => 'materialesdeaporte', $dat['Tungsteno']['id']))."<br />"; 
		  echo $this->Html->link('Editar', array('controller' => 'tungstenos', 'action' => 'edit', $dat['Tungsteno']['id'].'/'.str_replace(array('/', ':'), array('_', '-'), $this->request->url) ))."<br />"; 
		  ?></td>
        </tr>
<?php
}
...
'format' => __('Página %page% de %pages%, mostrando %current% tungstenos de lámina de %count% en total, empezando del calibre de lámina %start%, al %end%')
));
?>	</p>
	<div class="paging">
